...
# Modified 2017 xiou@microsoft.com. Changed normalization to [0,1].
#
...
"""Read and preprocess image data.
 ...
 decode_png: Decode a PNG encoded string into a 3-D float32 Tensor.
...
def regular_image(image, height, width, scope=None):

    with tf.name_scope(values=[image, height, width], name=scope,
                       default_name='regular_image'):
...
def image_preprocessing(image_buffer, bbox, train, thread_id=0, regular=False):
  ...
  if regular:
    ...
    image = regular_image(image, height, width)
  else:
    if train:
